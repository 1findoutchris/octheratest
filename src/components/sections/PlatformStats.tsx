...
export function PlatformStats() {
  return (
    <section className="relative border-y border-ink-soft/60 bg-obsidian-2/40">
      <Container>
        <div className="grid grid-cols-2 divide-x divide-y divide-ink-soft/60 lg:grid-cols-4 lg:divide-y-0">
          {PLATFORM_STATS.map((stat, i) => (
            <Reveal key={stat.label} delay={i * 0.08} y={16}>
              <div className="flex flex-col items-center gap-2 px-4 py-10 text-center sm:py-12">
                ...
                <p className="max-w-[16ch] text-sm text-mist">{stat.label}</p>
              </div>
            </Reveal>
          ))}
        </div>
