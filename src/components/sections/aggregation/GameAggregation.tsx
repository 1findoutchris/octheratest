...
export function GameAggregation() {
  return (
    <section id="aggregation" className="relative overflow-x-clip py-24 sm:py-32">
      <GlowOrb color="violet" className="left-1/2 top-0 h-[420px] w-[420px] -translate-x-1/2" />
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {AGGREGATION_FEATURES.map((feature, i) => (
            <Reveal key={feature.title} delay={(i % 3) * 0.08}>
              <div className="flex items-start gap-3.5 rounded-xl border border-ink-soft bg-white/[0.02] p-5">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-ink-soft bg-white/[0.03] text-cyan-soft">
                  <feature.icon className="h-5 w-5" strokeWidth={1.75} aria-hidden />
                </span>
                <div>
                  <p className="text-sm font-semibold text-frost">{feature.title}</p>
                  <p className="mt-1 text-sm leading-relaxed text-mist">{feature.description}</p>
                </div>
              </div>
            </Reveal>
          ))}
